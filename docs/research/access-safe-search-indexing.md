...
## Search-fork acceptance gates

Operating a forked/self-hosted search service is not an alternative way to defer the access seam. It is justified only when **all** of these gates have recorded evidence. Failing any gate retains the PostgreSQL authority and public-only external adapters.

1. **A named, accepted product requirement is blocked.** The requirement must name the observable and context that PostgreSQL-backed authorized projections cannot meet (for example, a committed scale, latency, language-analysis, or retrieval-quality target for Party-scoped search). "Better search" or a provider feature list is not evidence. The PostgreSQL prototype must run the agreed representative corpus and query mix and show the missed target reproducibly.
2. **The candidate is access-first by construction.** Its only indexed Party-scoped input is a published `SearchProjection` / `SearchEntitlement` stream from `WIKI.contentRead`; it never ingests a page model, complete render, GM Notes, or a client-supplied Party/role filter. The service query API accepts the server-resolved, immutable access context or a server-minted non-removable capability, and selects visible projections before match, rank, limit/cursor, deduplication, total, facet, highlight/snippet, suggestion, tag/property vocabulary, and relationship aggregation.
3. **A black-box noninterference suite passes.** For every pair of corpora that differ only in material not visible to a context, the complete response is identical for that context: status/missing shape, hit IDs/order/cursor, snippets/highlights, totals, facets, suggestions, tags/properties, relationship results, timing-bucket/error class where observable, and cache result. The fixture matrix includes anonymous, Player, GM, Admin, Player Preview, no/invalid/selected Party, multiple Party memberships, mixed-audience Entries, Secret/GM Notes, and a public crawler. The same suite must pass direct search, autocomplete/link-picker, related-content, and crawler/public-index endpoints.
4. **Revocation is demonstrably fail-closed.** A grant-to-relock test proves the former reader loses hits and every derived observable at the entitlement source immediately, with old cursor/cache/token requests rejected or recomputed by access epoch. If the fork has asynchronous replicas or index deletion, its acknowledged generation must be checked before serving; during uncertainty it returns a safe empty/unavailable result rather than a stale result. The measured end-to-end time must meet a written revocation SLO agreed before adoption.
5. **Projection and rebuild parity holds.** Event-driven writes, crash/retry replay, full rebuild, migration backfill, edit, publish, unpublish, deletion, public-to-restricted, restricted-to-public, grant, and relock produce the same reachable projection set and response as the PostgreSQL oracle. Reconciliation detects orphan, stale-generation, and over-shared documents and has a tested repair path.
6. **The operational cost is owned and bounded.** A design review records deployment, version/patch cadence, vulnerability response owner, backup/restore, observability redaction, tenant isolation, capacity plan, disaster recovery, upgrade/rollback, and a maintained compatibility suite. It also shows that the service does not create an unbounded per-Party/per-grant copy of audience-body text. A load/chaos test demonstrates the target requirement while preserving gates 2--5.
7. **The exit remains real.** KV-Wiki keeps the application-owned projection schema and a PostgreSQL reference implementation as the semantic oracle. The fork can be disabled without losing authoritative content or access state, and a documented, rehearsed rollback returns Party-aware reads to PostgreSQL without serving a stale public or privileged cache.

These gates distinguish a proven capacity/capability need from mere availability of Elasticsearch, Algolia, or another engine. They are deliberately stricter than a hit-filter demonstration: Elastic documents that document-level security ignores suggesters and can still expose aggregate information, illustrating why a service must be evaluated against every observable rather than only returned documents ([Elastic document-level security limitations](https://www.elastic.co/guide/en/elasticsearch/reference/current/document-level-security.html)).
